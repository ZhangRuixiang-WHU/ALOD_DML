...
class SelfsupLocHead(nn.Module):
    ...
    def forward(self, t_org_feat, t_aug_feat, s_org_feat,s_aug_feat, gt):
        ...
        if t_org_feat.shape[0] == 0:
            t_org_feat = torch.zeros(2,self.in_channels,device=t_org_feat.device,dtype=t_org_feat.dtype)
            t_aug_feat = torch.zeros(2,self.in_channels,device=t_aug_feat.device,dtype=t_aug_feat.dtype)
            s_org_feat = torch.zeros(2,self.in_channels,device=s_org_feat.device,dtype=s_org_feat.dtype)
            s_aug_feat = torch.zeros(2,self.in_channels,device=s_aug_feat.device,dtype=s_aug_feat.dtype)
        ...
        try:
            ...
            print('*********************************************')
            print(t_org_feat, t_aug_feat, s_org_feat,s_aug_feat, gt)
        # loss1 = self.criterion(pred_loc1, gt, reduction='mean')
