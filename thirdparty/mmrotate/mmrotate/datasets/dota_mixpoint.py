...
class DOTADataset_P(CustomDataset):
    ...
    def __init__(self,
        ...
        super(DOTADataset_P, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def load_annotations(self, ann_folder):
        ...
        if not ann_files:  # test phase
            ...
            for ann_file in ann_files:
                ...
                gt_points = []
                with open(ann_file.replace('annfiles_1ins','annfiles_point')) as f:
                    s = f.readlines()
                    for si in s:
                        bbox_info = si.split()
                        poly = np.array(bbox_info[:4], dtype=np.float32)
                        try:
                            px, py, cx, cy = poly
                        except:  # noqa: E722
                            continue
                ...
                if gt_bboxes:
                    ...
                    data_info['ann']['points'] = np.array(
                        gt_points, dtype=np.float32)
                    ...
                    data_info['ann']['points'] = np.zeros((0, 2),
                                                          dtype=np.float32)
    ...
    def evaluate(self,
                 ...
                 iou_thr= 0.5,#[0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95],#
                 scale_ranges=None,
                 nproc=4,
                 classwise = False):
        ...
        if metric == 'mAP':
            ...
            if isinstance(iou_thr, float):
                ...
                for iou_th in iou_thr:
                    ...
                    nproc=nproc)
                    sum_ap += mean_ap
                eval_results['mAP'] = sum_ap / len(iou_thr)
        else:
