...
from .aitodr_eval.aitodr_eval import AITODReval
import itertools
...
class AITODRDataset(CustomDataset):
    """aitodr dataset for detection.
    ...
    def __init__(self,
        ...
        super(AITODRDataset, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def evaluate(self,
        ...
        eval_results = {}
        AITODREval = AITODReval(annotations, results, numCats=len(self.CLASSES), nproc=nproc)
        AITODREval.params.iouThrs = iou_thrs
        ...
        AITODREval.evaluate()
        AITODREval.accumulate()
        AITODREval.summarize()

        classwise = False
        if classwise:  # Compute per-category AP
            # Compute per-category AP
            # from https://github.com/facebookresearch/detectron2/
            precisions = AITODREval.eval['precision']
        ...
        for metric_item in metric_items:
            ...
            val = float(
                f'{AITODREval.stats[aitodr_metric_names[metric_item]]:.3f}'
            )
            eval_results[key] = val
        ap = AITODREval.stats[:7]
