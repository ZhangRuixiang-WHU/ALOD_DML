...
def multiclass_nms_rotated(multi_bboxes,
    ...
    if bboxes.numel() == 0:
        ...
        if  return_ass_cent:
            assist_center = dets[:,:2].clone()
            if return_inds:
                return dets, labels, assist_center,inds
            else:
                return dets, labels, assist_center
        else:
            if return_inds:
    ...
    if not return_ass_cent:
        ...
        pseudo_bboxes = bboxes[keep]
        pseudo_scores = scores[keep]
        pseudo_labels = labels[keep]
        overlaps = iou_calculator(pseudo_bboxes, bboxes)

        assist_center = pseudo_bboxes[:,:2].clone()
        num_pseudo_bboxes = pseudo_bboxes.shape[0]
        sort_iou, sort_ind = overlaps.sort(1,True)

        for i in range(num_pseudo_bboxes):
            i_iou = sort_iou[i]
            i_ind = sort_ind[i]
            if mode == 'scr':
                ind_val_iou = i_ind[i_iou>0.8]
                ind_val_cls = ind_val_iou[scores[ind_val_iou].sort(0,True)[1]]
                ind_val = ind_val_cls[scores[ind_val_cls]>0.8]
                if ind_val.shape[0] <= 1:
                    pass
                else:
                    assist_box = bboxes[ind_val]
                    assist_center[i] = assist_box[1:,:2].mean(0)

        if return_inds:
            return torch.cat([pseudo_bboxes, pseudo_scores[:, None]], 1), pseudo_labels, assist_center, keep
        else:
            return torch.cat([pseudo_bboxes, pseudo_scores[:, None]], 1), pseudo_labels, assist_center


def multiclass_nms_rotated_sigma(multi_bboxes,
                                multi_sigmas,
                                multi_scores,
                                score_thr,
                                nms,
    ...
    if multi_bboxes.shape[1] > 5:
        bboxes = multi_bboxes.view(multi_scores.size(0), -1, 5)
        sigmas = multi_sigmas.view(multi_scores.size(0), -1, 5)
    else:
        bboxes = multi_bboxes[:, None].expand(
            multi_scores.size(0), num_classes, 5)
        sigmas = multi_sigmas[:, None].expand(
            multi_scores.size(0), num_classes, 5)
    scores = multi_scores[:, :-1]
    ...
    inds = valid_mask.nonzero(as_tuple=False).squeeze(1)
    bboxes, sigmas, scores, labels = bboxes[inds], sigmas[inds], scores[inds], labels[inds]

    sigma_mean = sigmas.sum(dim=-1) * 0.2
    sigmas = torch.sigmoid(sigma_mean)
    sigmas = (torch.ones_like(sigmas)-sigmas).view(-1,1)
    ...
    if return_inds:
        return torch.cat([bboxes, scores[:, None], sigmas], 1), labels, keep
    else:
        return torch.cat([bboxes, scores[:, None], sigmas], 1), labels
