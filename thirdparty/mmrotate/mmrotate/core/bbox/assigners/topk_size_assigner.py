...
class TopKSizeAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None, ast_bboxes=None):
        """Assign gt to bboxes.
        ...
        gt_size = gt_bboxes[:,2]*gt_bboxes[:,3]
        # gt_size = gt_size <400
        gt_size_1 = torch.where(gt_size > 4096, 1, 0)
        gt_size_2 = torch.where(gt_size > 1024, 1, 0)
        # gt_size_3 = torch.where(gt_size > 576, 1, 0)
        # gt_size_4 = torch.where(gt_size > 256, 1, 0)
        gt_size_5 = torch.where(gt_size <= 256, 2, 0)
        gt_size = gt_size_1 + gt_size_2+ gt_size_5 #+ gt_size_3 + gt_size_4 
        ...
        # assign_result = self.assign_wrt_size_ranking(overlaps, gt_size, gt_labels)
        assign_result = self.assign_wrt_size_ranking_test(overlaps, gt_size, gt_labels)
        if assign_on_cpu:
    ...
    def assign_wrt_size_ranking(self,  overlaps, gt_size, gt_labels=None):
    ...
    def assign_wrt_size_ranking_test(self,  overlaps, gt_size, gt_labels=None):
        num_gts, num_bboxes = overlaps.size(0), overlaps.size(1)
