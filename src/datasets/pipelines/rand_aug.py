    ...
    def _translate_bboxes(self, results, offset, direction="horizontal"):
        ...
        for key in results.get("bbox_fields", []):
            if results[key].shape[0] > 0:
                ...
                if direction == "horizontal":
                    # cx = np.maximum(0, cx + offset)
                    # cx = np.minimum(w, cx + offset)
                    cx = cx + offset
                elif direction == "vertical":
                    # cy = np.maximum(0, cy + offset)
                    # cy = np.minimum(h, cy + offset)
                    cy = cy + offset
                translate_coor = np.concatenate([cx, cy, cw, ch, angle], axis=-1)
                filter_flag = ((cx<w)&(cx>0)&(cy<h)&(cy>0)).squeeze(-1)
                translate_coor = translate_coor[filter_flag]
                # the boxes translated outside of image will be filtered along with
                # the corresponding masks, by invoking ``_filter_invalid``.
                results[key] = translate_coor
...
class RandRotate(GeometricAugmentation):
    ...
    def _rotate_bboxes(self, results, rotate_matrix):
        ...
        for key in results.get("bbox_fields", []):
            if results[key].shape[0]>0:
                coordinates_pbbs = obb2poly_le90(torch.tensor(results[key]))
                coordinates = coordinates_pbbs.reshape(coordinates_pbbs.shape[0],-1,2).unsqueeze(-1)
                coordinates = torch.cat([coordinates,torch.ones(coordinates.shape[0], 4, 1, 1)],dim=2)
                coordinates = np.array(coordinates)
                rotated_coords = np.matmul(rotate_matrix, coordinates)  # [nb_bbox, 4, 2, 1]
                rotated_coords = rotated_coords[..., 0]  # [nb_bbox, 4, 2]
                rotated_coords = poly2obb_le90(torch.tensor(rotated_coords))
                filter_flag=((0<rotated_coords[...,0])&(rotated_coords[...,0]<w)&(0<rotated_coords[...,1])&(rotated_coords[...,1]<h))
                rotated_coords = rotated_coords[filter_flag]
                rotated_coords = np.array(rotated_coords)
                results[key] = rotated_coords.astype(results[key].dtype)
            else:
                pass
...
class RandShear(GeometricAugmentation):
    ...
    def _shear_bboxes(self, results, magnitude, direction="horizontal"):
        ...
        for key in results.get("bbox_fields", []):
            if results[key].shape[0]>0:
                coordinates_pbbs = obb2poly_le90(torch.tensor(results[key]))
                coordinates = coordinates_pbbs.reshape(coordinates_pbbs.shape[0],-1,2)# [nb_bbox, 4, 2]
                coordinates = coordinates.transpose(1,2)# [nb_bbox, 2, 4]
                coordinates = np.array(coordinates)
                rotated_coords = np.matmul(shear_matrix[None, :, :], coordinates)  # [nb_bbox, 2, 4]
                rotated_coords = torch.tensor(rotated_coords).transpose(1,2) # [nb_bbox, 4, 2]
                rotated_coords = poly2obb_le90(rotated_coords)
                filter_flag=((0<rotated_coords[...,0])&(rotated_coords[...,0]<w)&(0<rotated_coords[...,1])&(rotated_coords[...,1]<h))
                rotated_coords = rotated_coords[filter_flag]
                rotated_coords = np.array(rotated_coords)
                results[key] = rotated_coords.astype(results[key].dtype)
        ...
        #         results[key], results[key].shape[-1], axis=-1
        #     )
        #     results[key] = np.stack([cx, cy, cw, ch, angle], axis=-1).astype(
        #         results[key].dtype
        #     )
...
class RandResize(transforms.Resize):
    ## change by zrx  modify to the RRandResize
...
class RandFlip(transforms.RandomFlip):
    ## change by zrx  modify to the RRandFilp
